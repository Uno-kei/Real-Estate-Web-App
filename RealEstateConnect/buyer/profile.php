...
// If database query fails, use demo buyer data
if (!$buyer) {
    $buyer = [
        'id' => $buyerId,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($fullName) || empty($phone)) {
        $updateError = 'Name and phone are required fields.';
    } elseif (!empty($newPassword) && $newPassword !== $confirmPassword) {
        $updateError = 'New password and confirmation do not match.';
    } elseif (!empty($newPassword) && !verifyPassword($currentPassword, $buyer['password'] ?? '')) {
        ...
        $params = [$fullName, $phone, $address, $city, $state, $zipCode, $buyerId];
        $types = "ssssssi";
        ...
        if (!empty($newPassword)) {
            ...
            updateData($sql, "si", [$passwordHash, $buyerId]);
        }
